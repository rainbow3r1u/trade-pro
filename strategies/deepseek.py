...
"""
    ...
    def scan(self) -> List[Dict[str, Any]]:
        ...
        return [{
            ...
            'tech_feature': item.get('tech_feature', '-')[:100],
            'expected_space': item.get('expected_space', '-')[:50],
            'entry_advice': item.get('entry_advice', '-')[:50],
            'indicator': f"[{item.get('stage', '-')}] {item.get('tech_feature', '-')[:80]}",
            'note': f"目标:{item.get('expected_space', '-')[:30]}; 入场:{item.get('entry_advice', '-')[:30]}"
        } for item in items]
