    ...
    def _pre_filter_symbols(self, df: pd.DataFrame, symbols: list) -> tuple:
        ...
        for symbol in symbols:
            ...
            if len(yesterday_data) > 0:
                ...
                yesterday_bullish = row['close'] > row['open']
        ...
        self.logger.info(f"日线预筛选: 总计 {len(symbols)} 个币种")
        self.logger.info(f"  - 今天+昨天均为阳线: {filter_stats['daily_bullish_pass']} 个")
        self.logger.info(f"  - 成交额>={min_volume_24h/1e6:.0f}M USDT: {filter_stats['volume_pass']} 个")
        self.logger.info(f"  - 通过预筛选: {filter_stats['both_pass']} 个")
        ...
        for idx, symbol in enumerate(passed_symbols, 1):
            ...
            for i, row in recent_6h.iterrows():
                ...
                if consecutive_count == 0:
                    ...
                    self.logger.info(f"  {symbol} STEP1: 阳线, 震幅{range_pct*100:.2f}%")
                    check_stats['step1'] += 1
                    ...
                    if current_low <= low_price:
                        self.logger.info(f"  {symbol} 第{i+1}根K线最低价未抬高: {low_price:.6f} <= {current_low:.6f}")
                        break
                    ...
                    self.logger.info(f"  {symbol} STEP{consecutive_count}: 阳线, 震幅{range_pct*100:.2f}%")
            ...
            if consecutive_count >= min_hours:
                bars_info = bars_info[::-1]
                
                first_bar = recent_6h.iloc[consecutive_count - 1]
                last_bar = recent_6h.iloc[0]
                
                total_gain = (last_bar['close'] - first_bar['open']) / first_bar['open'] * 100
                last_vol = last_bar['quote_volume']
                
                start_time = first_bar['timestamp']
