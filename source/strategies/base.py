...
from models import StrategyReport, Database
...
class BaseStrategy(ABC):
    ...
    def save_report(self, report: StrategyReport, save_to_db: bool = True) -> Path:
        json_output = report.to_dict()
        
        json_file = config.OUTPUT_DIR / f"{self.strategy_id}_{report.timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        with open(json_file, 'w', encoding='utf-8') as f:
            json.dump(json_output, f, ensure_ascii=False, indent=2)
        
        self.logger.info(f"报告已保存: {json_file}")
        
        if save_to_db and report.items:
            Database.save_signals_batch(
                strategy=self.strategy_id,
                signals=report.items,
    ...
    def create_report(self, items: List[Dict[str, Any]], 
                      ...
                      raw_analysis: str = None) -> StrategyReport:
