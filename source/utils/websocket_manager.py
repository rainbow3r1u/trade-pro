...
class BinanceWebSocketManager:
    ...
    def update_subscriptions(self, symbols: list):
        ...
        to_add = new_symbols - self.subscriptions
        to_remove = self.subscriptions - new_symbols
        ...
        logger.info(f"更新订阅: 新增{len(to_add)}, 移除{len(to_remove)}, 当前{len(self.subscriptions)}")
    ...
    async def _connect_binance(self):
        ...
        streams = [f"{symbol.lower()}@kline_1h" for symbol in self.subscriptions]
