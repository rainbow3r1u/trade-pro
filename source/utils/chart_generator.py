...
class ChartGenerator:
    ...
    @classmethod
    def _fetch_ohlcv(cls, symbol: str, timeframe: str = '1h', limit: int = 24, filter_incomplete: bool = False) -> Optional[pd.DataFrame]:
    ...
    def generate_triple_chart(cls, symbol: str) -> Optional[bytes]:
        ...
        cache_file = cache_dir / f"{symbol}_triple.png"
        ...
        df_1h = cls._fetch_ohlcv(symbol, '1h', 25, filter_incomplete=True)
        df_4h = cls._fetch_ohlcv(symbol, '4h', 13, filter_incomplete=True)
        ...
        cls._draw_candlestick(ax2, df_4h, f'{symbol} - 4H (12 candles)', linewidth=1.5)
        cls._draw_candlestick(ax3, df_1d, f'{symbol} - Daily (12 candles)', linewidth=2)
...
def generate_chart(symbol: str, output_path: str = None) -> Optional[bytes]:
    return ChartGenerator.generate_chart(symbol, output_path)


def generate_triple_chart(symbol: str) -> Optional[bytes]:
    return ChartGenerator.generate_triple_chart(symbol)


def generate_charts_batch(symbols: List[str]) -> int:
