...
def record_from_scan_results():
    ...
    try:
        ...
        if not Path(signal_file).exists():
            signal_file = str(config.DATA_DIR / 'all_signals.json')
        ...
        for signal in signals:
            ...
            if hours >= 6:
                ...
                try:
                    ...
                    start_time = datetime.strptime(f"{current_year}-{start_time_str}", '%Y-%m-%d %H:%M')
                    end_time = datetime.strptime(f"{current_year}-{end_time_str}", '%Y-%m-%d %H:%M')
