...
# 策略配置
BB_CLIMB_ENABLED = True        # 启用布林爬坡信号
BB_CANDIDATE_ENABLED = True    # 启用布林候选信号
SURGE_ENABLED = True           # 启用delta_q突增信号（需buy_ratio>=70%）
VOL_SURGE_ENABLED = True       # 启用15分钟成交量突增信号

# 信号阈值
BB_MIN_CONSECUTIVE_HOURS = 2   # 布林爬坡最少连续小时数
BB_CANDIDATE_MIN_HOURS = 2     # 布林候选最少蓄力小时数
SURGE_MIN_DELTA_Q = 500000     # delta_q突增最小值（USDT）
SURGE_MIN_BUY_RATIO = 0.70     # delta_q突增最小买入比（70%）
VOL_SURGE_MIN_RATIO = 2.0      # 15分钟成交量突增最小倍数
...
def calculate_margin_by_priority(signal_type: str, available_balance: float) -> float:
    ...
    优先级（从高到低）：
    1. BB_CLIMB_2h+ : 20 USDT (全仓) - 布林爬坡连续2小时
    2. BB_CAND_2h+  : 10 USDT (半仓) - 布林候选连续2小时
    3. VOL_SURGE_2x+ : 10 USDT (半仓) - 15分钟成交量突增2倍以上
    4. SURGE_70%+   :  5 USDT (1/4仓) - delta_q突增且buy_ratio>=70%
    """
    if signal_type.startswith("BB_CLIMB_"):
    ...
    elif signal_type.startswith("VOL_SURGE_"):
        return min(BASE_MARGIN * 0.5, available_balance)  # 半仓 10 USDT
...
def evaluate_and_open():
    ...
    if BB_CLIMB_ENABLED:
        ...
        for sig in signals:
            ...
            if consecutive >= BB_MIN_CONSECUTIVE_HOURS:
                ...
                if open_position(symbol, signal_type, entry_price, stop_loss_price):
                    opened = True
    
    # 2. 布林候选信号（优先级3）
    if BB_CANDIDATE_ENABLED and not opened:
        signals = get_bb_candidate_signals()
        for sig in signals:
            if get_positions_count() >= MAX_POSITIONS:
                break
            if get_available_balance() < 5:
            ...
            stop_loss_price = get_stop_loss_price(symbol, entry_price)
            near_hours = sig.get("consecutive_hours", 0)
            
            if near_hours >= BB_CANDIDATE_MIN_HOURS:
                signal_type = f"BB_CAND_{near_hours}h"
                if open_position(symbol, signal_type, entry_price, stop_loss_price):
                    opened = True
    
    # 3. Delta Q 突增信号（优先级4-5）
    if SURGE_ENABLED and not opened:
    ...
    # 4. 15分钟成交量突增信号（优先级5）
    if VOL_SURGE_ENABLED and not opened:
